    ...
    var image_plugin = {
      ...
      hide_arrows : function(visible, size, width, image_position) {
        ...
        if(current_object.current_position <= image_position) {
          ...
          if(current_object.current_position > (size - image_position)) {
            $('#next').hide();
            $('#previous').show();
      ...
      image_movement : function(visible, size, current_position, width, image_position) {
        ...
        if (current_position >= (size - (parseInt(visible, 10) - image_position))) {
              ...
              if (current_object.first_click === 0) {
                if(current_object.last_position == 0) {
                  ...
                  if ((parseInt(current_position, 10) - parseInt(current_object.last_position) ) > image_position ){
                    console.log('89');
                    position = (((parseInt(current_position, 10) - parseInt(current_object.last_position)) - (parseInt(visible) - (image_position - i))) * parseInt(width, 10));
                  }
                  else if(!(current_object.last_position >= (size - (image_position - 1)))){
                   console.log('893');
                   position = ((parseInt(visible) - i) + ((parseInt(current_position, 10) - parseInt(current_object.last_position))) * parseInt(width, 10)); 
                  }
                }
          ...
          position = ( ( parseInt (current_position, 10) - ((image_position - (image_position-current_position)) )) * parseInt(width, 10) );
          $("img").animate({"left": -position + "px"}, "slow");
        } else {
          console.log('else');
          current_object.last_position = current_position;
      ...
      scroll_movement : function(current_position, size, visible, list_width, image_position) {
        ...
        if (current_position > (size - parseInt(visible, 10) + 2)) {
            ...
            if (current_position === "" + ((parseInt(size,10) - i)) || current_position === ((parseInt(size,10) - i))) {
              if (current_object.first_click_scroll === 0) {
                if(current_object.last_position == 0) {
                  ...
                  console.log('2');
                  scroll_position = ((parseInt(current_position, 10) - parseInt(current_object.last_position)) * lt_width);
                }
          ...
          scroll_position = (parseInt(current_position, 10) - 3) * lt_width;
        } else if ((current_object.last_position >= (size - image_position)) && (current_object.last_position < current_position)) {
          console.log('4');
          scroll_position = (parseInt(current_position, 10) - parseInt(current_object.last_position)) * lt_width;
        } else if((current_object.last_position >= (size - image_position + 1)) && (current_object.last_position > current_position )) {
          console.log('5');
    ...
    $.fn.dinesh_picture = function(options) {
      ...
      return this.each(function () {
        ...
        $("#next").live('click', function(){
          ...
          if(!(current_object.current_position <= default_settings.image_position && current_object.last_position <= current_object.current_position)) {
        ...
        $("#previous").live('click', function(){
          ...
          current_object.first_click_scroll = 0;
          current_object.scroll_position = 0;
          if(!((current_object.last_position >= ((size - default_settings.image_visible) + 1 + 2)) && (current_object.current_position>= ((size - default_settings.image_visible) + 1 + 2)))) {
            $("img").animate({"left": '+=' + default_settings.image_width}, "slow");
            image_plugin.scroll_movement(current_object.current_position, size, default_settings.image_visible, default_settings.list_width, default_settings.image_position);
          }
          image_plugin.remove_previous_position();
